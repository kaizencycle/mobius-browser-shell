...
import { ChevronRight, Download, Brain, Target } from 'lucide-react';
...
const INSIGHT_META: Record<JADEGraphInsight['type'], { label: string; color: string; bg: string; border: string }> = {
  dominant_theme:       { label: 'DOMINANT THEME',    color: 'text-blue-700',    bg: 'bg-blue-50',    border: 'border-blue-200' },
  neglected_area:       { label: 'NEGLECTED AREA',     color: 'text-amber-700',   bg: 'bg-amber-50',   border: 'border-amber-200' },
  emerging_cluster:     { label: 'EMERGING CLUSTER',   color: 'text-emerald-700', bg: 'bg-emerald-50', border: 'border-emerald-200' },
  suggested_connection: { label: 'SUGGESTED LINK',     color: 'text-purple-700',  bg: 'bg-purple-50',  border: 'border-purple-200' },
  knowledge_gap:        { label: 'KNOWLEDGE GAP',      color: 'text-red-700',     bg: 'bg-red-50',     border: 'border-red-200' },
};

const INSIGHT_TYPE_VALUES: JADEGraphInsight['type'][] = [
  'dominant_theme', 'neglected_area', 'emerging_cluster', 'suggested_connection', 'knowledge_gap',
];
    ...
    <div className={`rounded-xl border ${meta.border} ${meta.bg} p-4 space-y-2`}>
      ...
      {insight.relatedNodes.length > 0 && (
        <div className="flex flex-wrap gap-1.5 pt-1">
          {insight.relatedNodes.slice(0, 4).map((n) => (
            <span key={n} className="px-2 py-0.5 rounded-full bg-white/80 border border-slate-200 text-[10px] text-slate-500 font-mono">{n}</span>
      ...
      {insight.actionable && (
        <div className="flex items-center justify-between pt-1 gap-2">
          ...
          {onAct && (
            <button type="button" onClick={() => onAct(insight)} className={`shrink-0 px-3 py-1 rounded-lg text-[11px] font-semibold transition-colors ${meta.color} ${meta.bg} border ${meta.border} hover:opacity-80`}>
            <button
              type="button"
...
interface DominantThemesProps { graph: KnowledgeGraph; }
...
export const DominantThemes: React.FC<DominantThemesProps> = ({ graph }) => {
  ...
  if (entries.length === 0) return <div className="text-xs text-slate-400 italic">No domain data yet.</div>;
  return (
    <div className="space-y-3">
      <div className="text-xs font-bold uppercase tracking-wider text-slate-400 flex items-center gap-2">
        <Brain className="w-3.5 h-3.5" />Domain Weight Distribution
      ...
      {entries.map(([domain, weight]) => (
        <div key={domain} className="space-y-1">
          ...
          <div className="h-1.5 w-full bg-slate-800 rounded-full overflow-hidden">
            <div className="h-full rounded-full transition-all duration-500" style={{ width: `${(weight / max) * 100}%`, backgroundColor: DOMAIN_COLORS[domain] }} />
            <div
...
interface NeglectedAreaAlertProps { insights: JADEGraphInsight[]; }
...
export const NeglectedAreaAlert: React.FC<NeglectedAreaAlertProps> = ({ insights }) => {
  ...
  return (
    <div className="flex items-start gap-3 px-4 py-3 rounded-xl bg-amber-500/10 border border-amber-500/30 text-sm">
      ...
      <div>
        <div className="font-semibold text-amber-300 mb-1">{neglected.length} neglected area{neglected.length > 1 ? 's' : ''} detected</div>
        <div className="text-xs text-amber-200/70">Some concepts haven't been engaged with recently. Revisiting them can deepen your understanding.</div>
...
export const InsightTypeFilter: React.FC<InsightTypeFilterProps> = ({ selected, onChange, counts }) => (
  <div className="flex flex-wrap gap-2">
    <button type="button" onClick={() => onChange('all')} className={`px-3 py-1 rounded-full text-[11px] font-semibold transition-colors border ${selected === 'all' ? 'bg-slate-200 text-slate-800 border-slate-300' : 'bg-slate-800 text-slate-400 border-slate-700 hover:border-slate-500'}`}>
      All
    </button>
    {INSIGHT_TYPE_VALUES.map((type) => {
      ...
      return (
        <button key={type} type="button" onClick={() => onChange(type)} className={`px-3 py-1 rounded-full text-[11px] font-semibold transition-colors border flex items-center gap-1 ${active ? `${meta.bg} ${meta.color} ${meta.border}` : 'bg-slate-800 text-slate-400 border-slate-700 hover:border-slate-500'}`}>
          {meta.label.split(' ')[0]}
          {count > 0 && <span className={`ml-1 ${active ? meta.color : 'text-slate-500'}`}>{count}</span>}
        </button>
      );
    })}
  </div>
...
interface LearningPathCardProps { insight: JADEGraphInsight; onStart?: () => void; }
...
export const LearningPathCard: React.FC<LearningPathCardProps> = ({ insight, onStart }) => {
  ...
  return (
    <div className="rounded-xl border border-red-500/30 bg-red-500/5 p-4 space-y-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {insight.relatedNodes.map((node, i) => (
            <React.Fragment key={node}>
              ...
              {i < insight.relatedNodes.length - 1 && <ChevronRight className="w-3 h-3 text-slate-500 shrink-0" />}
      ...
      <button type="button" onClick={onStart} className="flex items-center gap-1.5 text-xs text-emerald-400 hover:text-emerald-300 font-medium transition-colors">
      <button
        type="button"
        onClick={onStart}
...
interface IntentAlignmentPanelProps { intents: IntentHorizon[]; graph: KnowledgeGraph; }
...
export const IntentAlignmentPanel: React.FC<IntentAlignmentPanelProps> = ({ intents, graph }) => {
  ...
  const withAlignment = intents.map((intent) => ({
    intent,
    aligned: intent.relatedConcepts.some((c) => graphNodeIds.has(c)),
  }));
  if (withAlignment.length === 0) return (
    <div className="text-xs text-slate-400 italic text-center py-4">No intent horizons set. Add intentions in the Knowledge Graph.</div>
  );
  return (
    <div className="space-y-2">
      <div className="text-xs font-bold uppercase tracking-wider text-slate-400 flex items-center gap-2"><Target className="w-3.5 h-3.5" />Intent Alignment</div>
export const JadeThinkingState: React.FC = () => {
  return (
    <div className="flex flex-col items-center justify-center gap-4 py-12">
      <div className="relative w-16 h-16">
    ...
    <div className="space-y-2">
      ...
      {withAlignment.map(({ intent, aligned }) => (
        <div key={intent.id} className="flex items-center justify-between gap-3 px-3 py-2 rounded-lg bg-slate-800/60 border border-slate-700/50">
          ...
          <span className={`shrink-0 px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wide ${aligned ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-amber-500/20 text-amber-400 border border-amber-500/30'}`}>
          <span
...
interface InsightHistoryEntry { timestamp: string; count: number; }
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        ...
        {newSinceLast > 0 && <span className="px-2 py-0.5 rounded-full bg-red-500/20 text-red-400 text-[10px] font-bold border border-red-500/30">+{newSinceLast} new</span>}
      </div>
      {recent.length === 0 ? <div className="text-xs text-slate-500 italic">No analysis runs yet.</div> : (
        <div className="space-y-1">
          {recent.map((entry, i) => (
            <div key={i} className="flex items-center justify-between text-xs text-slate-400">
              <span className="font-mono text-[10px]">{new Date(entry.timestamp).toLocaleString('en-US', { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' })}</span>
        {newSinceLast > 0 && (
...
interface WeeklyJADEReportProps { insights: JADEGraphInsight[]; graph: KnowledgeGraph; }
...
export const WeeklyJADEReport: React.FC<WeeklyJADEReportProps> = ({ insights, graph }) => {
  ...
  const newConcepts = graph.nodes.filter((n) => new Date(n.firstSeen) >= oneWeekAgo).length;
...
0        <div><div className="text-2xl font-mono text-emerald-400">{newConcepts}</div><div className="text-[10px] text-slate-400 mt-0.5">new concepts</div></div>
        <div><div className="text-2xl font-mono text-amber-400">{neglectedCount}</div><div className="text-[10px] text-slate-400 mt-0.5">neglected areas</div></div>
        <div><div className="text-2xl font-mono text-purple-400">{emergingCount}</div><div className="text-[10px] text-slate-400 mt-0.5">emerging clusters</div></div>
        <div>
...
interface ConfidenceScoreBarProps { confidence: number; compact?: boolean; }
...
export const ConfidenceScoreBar: React.FC<ConfidenceScoreBarProps> = ({ confidence, compact = false }) => {
  ...
  if (compact) return <span className={`text-[10px] font-mono font-bold ${textColor}`}>{pct}%</span>;
  return (
...
interface InsightActCTAProps { insight: JADEGraphInsight; onNavigate: (chamber: string) => void; }
...
export const InsightActCTA: React.FC<InsightActCTAProps> = ({ insight, onNavigate }) => {
  ...
  return (
    <button type="button" onClick={() => onNavigate(dest.chamber)} className="flex items-center gap-2 text-xs font-medium text-emerald-400 hover:text-emerald-300 transition-colors">
      <ChevronRight className="w-3.5 h-3.5" />{dest.label}
...
interface GraphDiffViewProps { graph: KnowledgeGraph; }
...
export const GraphDiffView: React.FC<GraphDiffViewProps> = ({ graph }) => {
  ...
  useEffect(() => {
    try {
      ...
      if (prev !== null) setAddedSinceLast(Math.max(0, graph.nodes.length - prev));
    } catch { setAddedSinceLast(0); }
  }, [graph.nodes.length]);
      if (prev !== null) {
...
export function snapshotGraphSize(size: number): void {
  try { localStorage.setItem(GRAPH_SNAPSHOT_KEY, JSON.stringify(size)); } catch { /* ignore */ }
  try {
...
export const CrossChamberSynthesis: React.FC = () => (
  <div className="rounded-xl border border-cyan-500/20 bg-cyan-500/5 px-4 py-3 text-xs text-cyan-300 leading-relaxed">
    ...
    <p>Your reflection moods correlate with your learning performance. Periods of exploratory reflection tend to precede higher OAA accuracy scores — consider deepening your reflection practice during study cycles.</p>
  </div>
);

// ── JADE-14: JADEEmptyState ───────────────────────────────────────────────────

interface JADEEmptyStateProps { onNavigate?: (chamber: string) => void; }

export const JADEEmptyState: React.FC<JADEEmptyStateProps> = ({ onNavigate }) => (
  <div className="flex flex-col items-center justify-center gap-4 py-12 text-center">
    <div className="text-4xl">🌱</div>
    <p className="text-slate-400 text-sm max-w-xs">Your knowledge graph is empty. Begin by completing an OAA module or writing a Reflection to build your graph.</p>
    <div className="flex gap-3">
      <button type="button" onClick={() => onNavigate?.('oaa')} className="px-4 py-2 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-xs font-medium hover:bg-emerald-500/20 transition-colors">📚 Go to OAA</button>
      <button type="button" onClick={() => onNavigate?.('reflections')} className="px-4 py-2 rounded-xl bg-purple-500/10 border border-purple-500/30 text-purple-400 text-xs font-medium hover:bg-purple-500/20 transition-colors">✨ Write Reflection</button>
    </div>
  </div>
);
...
interface SuggestedConnectionCardProps { insight: JADEGraphInsight; }
...
export const SuggestedConnectionCard: React.FC<SuggestedConnectionCardProps> = ({ insight }) => {
  ...
  return (
    <div className="rounded-xl border border-purple-500/30 bg-purple-500/5 p-4">
      <div className="text-[10px] font-bold uppercase tracking-widest text-purple-400 mb-3">Suggested Connection</div>
      <div className="flex items-center justify-center gap-3">
        <span className="px-3 py-1.5 rounded-lg bg-purple-500/20 border border-purple-500/30 text-purple-200 text-xs font-mono">{nodeA}</span>
        <span className="text-purple-400 font-bold">→</span>
        <span className="px-3 py-1.5 rounded-lg bg-purple-500/20 border border-purple-500/30 text-purple-200 text-xs font-mono">{nodeB}</span>
      </div>
      {insight.description && <div className="text-xs text-slate-400 mt-3 text-center">{insight.description}</div>}
export const CrossChamberSynthesis: React.FC = () => {
  return (
    <div className="rounded-xl border border-cyan-500/20 bg-cyan-500/5 px-4 py-3 text-xs text-cyan-300 leading-relaxed">
      <div className="font-bold uppercase tracking-wider text-cyan-400 text-[10px] mb-1">Cross-Chamber Synthesis</div>
  ...
  <div className="space-y-2">
    ...
    {DEMO_CONCEPTS.map((c) => (
      <div key={c.label} className="flex items-start gap-3 px-3 py-2.5 rounded-lg bg-slate-800/60 border border-slate-700/50">
        <div className="w-2.5 h-2.5 rounded-full mt-0.5 shrink-0" style={{ backgroundColor: DOMAIN_COLORS[c.domain] }} />
        <div>
...
interface InsightCountBadgeProps { count: number; }
...
export function getUnreadInsightCount(): number {
  try { const raw = localStorage.getItem(UNREAD_KEY); return raw ? parseInt(raw, 10) : 0; } catch { return 0; }
}

export function setUnreadInsightCount(count: number): void {
  try { localStorage.setItem(UNREAD_KEY, String(count)); } catch { /* ignore */ }
  try {
...
export function exportJADEReport(insights: JADEGraphInsight[], graph: KnowledgeGraph): void {
  ...
  insights.forEach((insight, i) => {
    ...
    if (insight.actionable) { lines.push(''); lines.push(`**Action:** ${insight.actionable}`); }
    if (insight.relatedNodes.length > 0) { lines.push(''); lines.push(`**Related nodes:** ${insight.relatedNodes.join(', ')}`); }
    lines.push('');
  });
...
interface ExportJADEReportButtonProps { insights: JADEGraphInsight[]; graph: KnowledgeGraph; }

export const ExportJADEReportButton: React.FC<ExportJADEReportButtonProps> = ({ insights, graph }) => (
  <button type="button" onClick={() => exportJADEReport(insights, graph)} disabled={insights.length === 0} className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-slate-800 border border-slate-700 text-slate-300 text-xs font-medium hover:bg-slate-700 transition-colors disabled:opacity-40 disabled:cursor-not-allowed">
...
export const JADEConversationMode: React.FC<JADEConversationModeProps> = ({ graph, onSend, disabled = false }) => {
  ...
  const topDomain = useCallback((): string => {
    const weights: Partial<Record<KnowledgeDomain, number>> = {};
    graph.nodes.forEach((n) => { weights[n.domain] = (weights[n.domain] ?? 0) + n.weight; });
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      />
      <button type="button" onClick={handleSend} disabled={disabled || !input.trim()} className="px-4 py-2 rounded-xl bg-emerald-500/20 border border-emerald-500/30 text-emerald-400 text-xs font-semibold hover:bg-emerald-500/30 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
      <button
        type="button"
        onClick={handleSend}
