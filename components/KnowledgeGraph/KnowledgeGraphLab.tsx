...
export const KnowledgeGraphLab: React.FC<KnowledgeGraphLabProps> = ({
  ...
  // Update dimensions on resize / layout (sidebar open changes width)
  useEffect(() => {
    const updateDimensions = () => {
      if (containerRef.current) {
        const { width, height } = containerRef.current.getBoundingClientRect();
        setDimensions({ width, height: Math.max(120, height - 60) });
      }
    };
  ...
  }, [selectedNodeId, graph.nodes.length]);
  ...
  useEffect(() => {
    if (!showJadeInsights) return;
    const onKey = (e: KeyboardEvent) => {
      if (e.key === 'Escape') setShowJadeInsights(false);
    };
  ...
  if (graph.nodes.length === 0) {
    return (
      <div className="h-full flex flex-col bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 text-slate-50">
        <a
          href="#kg-graph-region"
          className="sr-only focus:not-sr-only focus:absolute focus:left-4 focus:top-24 focus:z-50 focus:px-3 focus:py-2 focus:rounded-md focus:bg-emerald-600 focus:text-slate-950 focus:text-sm"
        >
          Skip to graph region
        </a>
        {/* Header */}
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <button
              ...
              className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-gradient-to-r from-emerald-500 to-cyan-500 text-slate-950 font-medium text-sm hover:from-emerald-400 hover:to-cyan-400 transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950"
            >
  ...
  return (
    <div className="h-full flex flex-col bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 text-slate-50 overflow-hidden relative">
      ...
      <div className="flex items-center justify-between px-4 sm:px-6 py-3 border-b border-slate-700/50 bg-slate-900/80 flex-none">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 hover:bg-emerald-500/20 transition-colors disabled:opacity-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400"
          >
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400 ${
          ...
          <button
            type="button"
            onClick={() => setShowIntentPanel(true)}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-gradient-to-r from-emerald-500/20 to-cyan-500/20 text-emerald-400 border border-emerald-500/30 hover:from-emerald-500/30 hover:to-cyan-500/30 transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400"
          >
      ...
      {/* Main Content Area */}
      <div id="kg-graph-region" className="flex-1 flex min-h-0 relative scroll-mt-4" ref={containerRef}>
        {/* Graph Visualization */}
      ...
      {showJadeInsights && (
        <div
          className="absolute inset-0 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4 z-50"
          role="dialog"
          aria-modal="true"
          aria-labelledby="jade-insights-title"
        >
          <div className="bg-slate-900 border border-emerald-500/30 rounded-xl max-w-lg w-full max-h-[80vh] overflow-hidden shadow-2xl shadow-emerald-500/10">
            <div className="flex items-center justify-between p-4 border-b border-slate-700">
              <div className="flex items-center gap-2">
                ...
                <h2 id="jade-insights-title" className="text-lg font-semibold text-slate-100">
                  JADE Insights
                </h2>
              </div>
              <button
                ...
                className="text-slate-400 hover:text-slate-200 transition-colors px-2 py-1 rounded focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400"
                aria-label="Close JADE insights"
              >
                ×
              </button>
            </div>
