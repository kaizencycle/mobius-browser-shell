...
export const MOBIUS_OPEN_INQUIRY_EVENT = 'mobius-open-inquiry';
...
export function InquiryChatModal({ isOpen: externalIsOpen, onOpenChange }: InquiryChatModalProps) {
  ...
  // Labs / shell can open inquiry without prop drilling (e.g. OAA header).
  useEffect(() => {
    const open = () => {
      if (onOpenChange) onOpenChange(true);
      else setInternalIsOpen(true);
    };
    window.addEventListener(MOBIUS_OPEN_INQUIRY_EVENT, open);
    return () => window.removeEventListener(MOBIUS_OPEN_INQUIRY_EVENT, open);
  }, [onOpenChange]);
  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed bottom-6 right-6 w-14 h-14 bg-gradient-to-br from-blue-500 to-purple-500 rounded-full shadow-2xl flex items-center justify-center hover:scale-110 transition-transform z-40 group motion-reduce:transform-none"
        aria-label="New Inquiry"
        title="Start a new inquiry"
      >
        <MessageCircle className="w-6 h-6 text-white group-hover:scale-110 transition-transform" />
        <span className="absolute inset-0 rounded-full bg-blue-400 opacity-75 animate-ping pointer-events-none motion-reduce:hidden" />
      </button>
