...
import { fetchOAuthStatus, initiateOAuth, type OAuthProvider } from '../../services/OAuthService';
...
export function OAuthButtons({ className, onError }: OAuthButtonsProps) {
  ...
  const [available, setAvailable] = useState<{ github: boolean; google: boolean }>({
    github: false,
    google: false,
  });
  ...
  useEffect(() => {
    fetchOAuthStatus().then((status) => {
      setAvailable({ github: status.github, google: status.google });
    });
  }, []);
  ...
  const login = (provider: OAuthProvider) => {
    ...
    try {
      ...
      setPending(null);
      onError?.(err instanceof Error ? err.message : 'OAuth sign-in failed');
    }
  };
  ...
  return (
    <div className={className ?? 'hall-oauth'}>
      ...
      {available.google && (
        <button
          className="hall-oauth-btn hall-oauth-btn--google"
          onClick={() => login('google')}
          disabled={!!pending}
          aria-label="Continue with Google"
        >
          <svg viewBox="0 0 18 18" aria-hidden width="14" height="14">
