 ...
 *
 * A client-side hash-linked blockchain for MIC (Mobius Integrity Credits).
 * Every MIC earning event is recorded as a block with:
 *   - SHA-256 hash linking to the previous block
 *   - Timestamp, nonce, and transaction data
 *   - Chain integrity verification
 *
 * This runs entirely in the browser (Web Crypto API) and persists to localStorage.
 * On mainnet, this local chain would sync with the distributed ledger.
 */
...
export interface MICTransaction {
  source: string;          // e.g. "learning_module_completion"
  amount: number;          // MIC earned
...
export interface MICBlock {
  ...
  hash: string;
  nonce: number;
  merkleRoot: string;      // hash of all transactions in the block
}
...
async function mineBlock(
  ...
  while (true) {
    ...
    nonce++;
    // Safety cap (browser-friendly)
    if (nonce > 500_000) {
      // Fallback — accept the hash anyway (testnet)
      return { hash: candidate, nonce };
    }
  }
}
...
export function useMICBlockchain() {
  ...
  // ── Initialize chain (load or create genesis) ──
  useEffect(() => {
  ...
  // ── Add a new block with MIC transactions ──
  ...
  // ── Get balance for a specific recipient ──
  ...
  // ── Get transactions for a recipient ──
  const getTransactions = useCallback(
    (recipient: string): (MICTransaction & { blockIndex: number; blockHash: string; timestamp: string })[] => {
  ...
  // ── Get all unique recipients and their balances ──
  const getAllHolders = useCallback((): { recipient: string; balance: number; txCount: number }[] => {
    ...
    for (const block of chain) {
      for (const tx of block.transactions) {
        if (tx.recipient === 'mobius-system') continue; // skip genesis
...
export default useMICBlockchain;
